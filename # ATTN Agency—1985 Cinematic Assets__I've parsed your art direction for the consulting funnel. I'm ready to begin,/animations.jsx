...
function Stage({
  ...
  React.useEffect(() => {
    ...
    const measure = () => {
      ...
      const cw = el.clientWidth, ch = el.clientHeight - barH;
      // Only accept plausible dimensions; never lock in a zero-size read.
      if (cw >= 8 && ch >= 8) {
        const s = Math.min(cw / width, ch / height);
        setScale(Math.max(0.05, s)); // React bails out if unchanged
      }
      // Self-verifying watchdog: async mounts (x-import, device frames) can
      // settle layout after first paint without ever resizing this element,
      // so re-measure every frame for the first ~2s regardless.
      if (frames++ < 120) raf = requestAnimationFrame(measure);
    };
    measure();
